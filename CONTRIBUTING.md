...
```
BUNDLE_GEMFILE=gemfiles/rails_8.1.rb \
...
mysql -e 'drop database authlogic; create database authlogic;' && \
  DB=mysql BUNDLE_GEMFILE=gemfiles/rails_8.1.rb bundle exec rake
```
...
DB=postgres BUNDLE_GEMFILE=gemfiles/rails_8.1.rb bundle exec rake
```
...
```
BUNDLE_GEMFILE=gemfiles/rails_8.1.rb bundle exec rubocop
```

To run the tests without linting, use `rake test`.

```
BUNDLE_GEMFILE=gemfiles/rails_8.1.rb bundle exec rake test
```
